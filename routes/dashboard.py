...
async def get_dashboard(
    ...
    def get_price_on(days_ago: int):
        ...
        # 1. Тухайн өдрийн бүх candle-г шүүх
        same_day_candles = [p for p in price_data if p["label"].startswith(target_str)]

        # 2. Олдохгүй бол хамгийн ойрын өмнөх candle-г авах
        if not same_day_candles:
            # target_date-аас өмнөх хамгийн ойрын дата
        ...
        # 3. Олдсон бол хамгийн сүүлийнхийг авах
        return same_day_candles[-1]["value"]

    today_price = await get_current_price()  # ⬅️ яг одоогийн үнэ авах
    history_ranges = [30, 60, 90]  # Today-г loop-д хийхгүй
    price_history = [
        {
            "label": "Today",
            "amountChange": 0.0,
    ...
    return {
        ...
        "insight": insight,
        "price": price_data,
        "price_history": price_history,  # ⬅️ шинэ талбар
